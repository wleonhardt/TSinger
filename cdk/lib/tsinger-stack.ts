...
const ORIGIN_SECRET_HEADER = "x-tsinger-origin-secret";
...
const CONTAINER_NAME = "tsinger";
const ORIGIN_LOG_RETENTION = logs.RetentionDays.ONE_MONTH;

export interface TsingerStackProps extends cdk.StackProps {
  ecrRepoName?: string;
  serviceName?: string;
...
function resolveImageIdentifier(repoUri: string, tag?: string, digest?: string): string {
  ...
  throw new Error(
    "Missing image reference. Provide imageTag (for example -c imageTag=0.1.0) or imageDigest."
  );
}
...
export class TsingerStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: TsingerStackProps) {
    ...
    const repo = createRepository
      ? new ecr.Repository(this, "TsingerRepo", {
      ...
      : ecr.Repository.fromRepositoryName(this, "TsingerRepo", ecrRepoName);
    ...
    const vpc = new ec2.Vpc(this, "TsingerVpc", {
      maxAzs: 1,
      natGateways: 0,
      subnetConfiguration: [
        {
    ...
    const originSecurityGroup = new ec2.SecurityGroup(this, "OriginSecurityGroup", {
      ...
      description:
        "TSinger EC2 origin security group. Inbound traffic is limited to CloudFront origin-facing infrastructure.",
    });
    ...
    const instanceRole = new iam.Role(this, "TsingerInstanceRole", {
      assumedBy: new iam.ServicePrincipal("ec2.amazonaws.com"),
      description: "Allows the TSinger EC2 origin to pull from ECR and be managed through SSM.",
    ...
    const instance = new ec2.Instance(this, "TsingerOriginInstance", {
      vpc,
    ...
    if (enableBasicAuth && basicAuthUsername && basicAuthPassword) {
      ...
      const authFunctionCode = cdk.Fn.join("", [
        ...
        "      headers: {\n",
        '        "www-authenticate": { value: \'Basic realm="TSinger"\' },\n',
      ...
      rules: [
        {
          ...
          visibilityConfig: {
            cloudWatchMetricsEnabled: true,
            metricName: "TsingerEdgeWafCommonRules",
            sampledRequestsEnabled: true,
          },
          statement: {
        ...
        },
        {
          ...
          visibilityConfig: {
            cloudWatchMetricsEnabled: true,
            metricName: "TsingerEdgeWafRateLimit",
            sampledRequestsEnabled: true,
          },
          statement: {
            rateBasedStatement: {
    ...
    const distribution = new cloudfront.Distribution(this, "TsingerCdn", {
    ...
    const cloudFront5xxAlarm = new cloudwatch.Alarm(this, "CloudFront5xxAlarm", {
      ...
      alarmDescription: "TSinger CloudFront 5xx error rate is elevated.",
    });
    ...
    const ec2StatusCheckAlarm = new cloudwatch.Alarm(this, "Ec2StatusCheckFailedAlarm", {
      ...
      treatMissingData: cloudwatch.TreatMissingData.NOT_BREACHING,
      alarmDescription: "The TSinger EC2 origin has failed an EC2 status check.",
    });
    ...
    new cdk.CfnOutput(this, "CloudFrontUrl", {
      ...
      description: "CloudFront URL — primary entry point for TSinger.",
    });
    ...
    new cdk.CfnOutput(this, "InstanceId", {
      ...
      description: "EC2 instance ID for the TSinger origin.",
    });
    ...
    new cdk.CfnOutput(this, "EcrRepositoryUri", {
      ...
      description: "ECR repository URI for pushing TSinger images.",
    });
    ...
    new cdk.CfnOutput(this, "OriginLogGroupName", {
      ...
      description: "CloudWatch Logs group containing TSinger origin container logs.",
    });
  }
}
